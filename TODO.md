...
- [ ] 🟡 ★ `M1-GATE-EXT` **GATE**: per-venue both-dir differential GREEN for the 3 new venues (extends M1-GATE; LiteSVM real `.so`) *(testing · 4d · onchain-17,18,19, sizing-12,13,14)* — off-chain quoters/decoders/adapters all ready; **the real-venue-in-LiteSVM harness is now PROVEN** (Raydium CPMM, `real_venue_raydium_cpmm.rs`/`dump_raydium_cpmm.py` — load real `.so` + snapshot accounts + warp Clock). Each Fase-2.5 venue is a bounded extension of that template: DAMM v2 (single-range sqrt-price, no tick arrays) is nearest; Raydium CLMM needs tick-array snapshots; DLMM needs bin-array snapshots. **Not yet run per-venue** — the template + dumper exist to do so
