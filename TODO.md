...
Workspace is **green**: `cargo build/test/clippy -D warnings/fmt` pass (**234 host tests** + 16 new arb-math whirlpool tests (sizing-5) + the LiteSVM M1-GATE suite, host + WSL), `config-check` validates. **Fase 0 is now COMPLETE (20/20)** — Wave-1 program IDs verified on-chain, M0 milestone green. **detection-3 CLOSED** — all 3 Wave-1 venue decoders (Whirlpool / Raydium CP-Swap / PumpSwap) verified against real mainnet accounts (real-bytes tests).
...
> **2026-06-22 (cont.) — `sizing-5` Orca Whirlpool bit-exact OFF-CHAIN quoter landed.** New `crates/arb-math/whirlpool.rs` (`WhirlpoolPool::quote_exact_in`) is a verbatim port of Orca's on-chain `compute_swap` + sqrt-price/amount-delta integer math (Q64.64), BOTH directions, Floor-output/Ceil-input, fee-on-input floor, `CrossesTick` at the supplied next-tick `sqrt_price_limit`. 16 new host tests (39 arb-math total) incl. hand-verified power-of-two fixtures + a lossless fee-0 round-trip; clippy `-D warnings`/fmt/`no_std` clean; arb-bot 157 tests still green. A 4-lens adversarial workflow (rounding/direction · 256-bit overflow · fee/CrossesTick · edge-cases) confirmed it **bit-exact** vs canonical. This closes the OFF-CHAIN side of the Orca M1-GATE residual; the on-chain `swap_v2` CPI differential (onchain-7 + Orca LiteSVM, needs build-sbf) remains.

> **2026-06-22 (cont.) — M1-GATE LiteSVM side substantially CLOSED for Raydium CPMM (Class A).** New `tests/litesvm-tests/tests/`: `rounding_mirror_fuzz.rs` (BOTH directions + 256-case fuzz over reserves+fee+amount, 0 drift, shrink-to-minimal), `t22_fee.rs` (Token-2022 receipt-fee path differential), `litesvm_unit.rs` (revert/exact-delta/boundary), `trust_boundary.rs` (6001/6002), `token2022_filter.rs` (7 BadExt), `closure.rs` (add-2 on-chain `RouteDoesNotClose`=6010), `cu_budget.rs` (measured CU<1.4M); shared `tests/common/mod.rs` single-sources the builders. **Closed: onchain-9, testing-4, testing-6, add-2.** Advanced onchain-10/sizing-9/testing-5 (Raydium CPMM both-dir+fuzz+T22) and add-5/testing-7 (measured CU). On-chain `processor` gained the add-2 closure assert; `arb-types` gained `RouteDoesNotClose=6010`; the swap-harness gained per-pool fee + Token-2022 receipt-fee skim (bit-identical to `arb_math::fees`). **Residual for FULL M1-GATE: Orca sqrt-price venue + Surfpool real-venue rounding (Class B).**
...
- [ ] 🟡 ★ `onchain-6` Raydium CPMM swap adapter *(onchain · 3d · onchain-3,4,5)* — adapter + Anchor `swap_base_input` discriminator filled (CONFIRMED on-chain: real Raydium dispatches it, 4391 CU); CP math proven via LiteSVM M1-GATE; real-venue differential harness BUILT (`surfpool_real_venue.rs`: real pool decode + reserve=vault−protocol−fund + arb_math quote verified vs LIVE mainnet fork) — but the live swap is BLOCKED: real Raydium reverts `InvalidAccountData` @4391 CU inside Anchor account validation on the surfpool fork (raw zero-copy/spl deserialize over forked-account data, no Anchor log, amount-independent = surfpool substrate limit, NOT a math defect). Alt path = real Raydium `.so` in LiteSVM. 🔒
...
- [ ] 🟡 ★ `onchain-10` Rounding-mirror fuzz/property gate (per-venue, both dirs) — **M1-GATE** *(onchain · 4d · onchain-9, sizing-8)* — Raydium CPMM venue GREEN via LiteSVM: both directions + 256-case fuzz over reserves+fee+amount (0 drift, shrink-to-minimal) + Token-2022 receipt-fee path (rounding_mirror_fuzz.rs + t22_fee.rs). Orca off-chain mirror now DONE (sizing-5, audited bit-exact); residual: on-chain Orca `swap_v2` adapter (onchain-7) + the Orca LiteSVM differential + Surfpool real-venue rounding (onchain-11)
- [ ] 🟡 `onchain-11` Surfpool mainnet-fork integration test (revert on real programs) *(onchain · 3d · onchain-10)* — **runtime-PARITY GREEN** (surfpool_integration.rs): arb_program loaded via `surfnet_setAccount` executes on the REAL agave runtime (solana-core 2.2.4); M1-GATE differential realized==predicted + Unprofitable(6000) revert both hold. Fork via run_surfpool.sh (keyed Chainstack datasource). Remaining: real-VENUE differential (clone a real Raydium pool + the real swap CPI account list = onchain-6)
...
- [x] `sizing-5` Orca Whirlpool Quoter (bit-exact, in-range) *(sizing · 2.5d · sizing-3)* — `crates/arb-math/whirlpool.rs`: verbatim port of Orca `compute_swap` + sqrt-price/amount-delta math (Q64.64), BOTH directions, Floor-output/Ceil-input per dir, fee-on-input floor, `CrossesTick` when the post-fee input would reach the supplied next-tick `sqrt_price_limit` (resolved off-chain via add-6). 16 host tests incl. hand-verified power-of-two fixtures + lossless fee-0 round-trip; uses arb-math `U256` (256-bit-overflow-equivalent to Orca `U256Muldiv`). 4-lens adversarial audit vs canonical Orca source = **bit-exact** (no floor/ceil flips, no direction misroute). Residual to fully close Orca on M1-GATE: on-chain `swap_v2` CPI differential (onchain-7 + sizing-9/onchain-10/testing-5, needs build-sbf).
...
- [ ] 🟡 `sizing-9` GATE: per-venue both-direction differential/property test *(sizing · 2.5d · sizing-8, onchain-9)* — part of M1-GATE; Raydium CPMM both-dir + fuzz(reserves+fee+amount) + Token-2022 GREEN (LiteSVM); Orca off-chain quoter DONE (sizing-5); residual = the Orca on-chain CPI differential + Surfpool real-venue

### detection
- [x] `detection-2` Core model + SessionStamp dedupe types *(detection · 1d · detection-1)*
- [x] `detection-3` Per-venue decoders (CPMM vaults+PoolState, Whirlpool, PumpSwap) *(detection · 4d · detection-2)* — **ALL 3 Wave-1 venue decoders VERIFIED against canonical structs + REAL mainnet accounts** (real-bytes `include_bytes!` tests over frozen `getAccountInfo` fixtures; commits `f1c88ca`/`952e2cf`/this): **Whirlpool** (sqrt_price@65, liquidity@49, tick_current@81, fee_rate@45, tick_spacing@41, mints+vaults) via 3 SOL/USDC pools (tick↔sqrt mutually consistent); **Raydium CP-Swap** PoolState (vaults/mints/amm_config) + AmmConfig `trade_fee_rate@12` (=2500 → 0.25%, denom 1e6); **PumpSwap** Pool (base_mint@43/quote_mint@75/pool_base_ta@139/pool_quote_ta@171/lp_supply@203/coin_creator@211) via a real pool reached through a swap's ALT-resolved accounts — pool token-accounts' `mint@0` == base/quote mint (mutual cross-check pins every offset). Each `decode_*` returns a typed struct + reserves/quoter bridge. (PumpSwap GlobalConfig fee read = Fase-2 sizing-6.)
...
- [x] `testing-4` LiteSVM unit tests: revert + exact-delta + boundary *(testing · 3d · testing-2,3, sizing-4, onchain-8)* — GREEN (litesvm_unit.rs): unprofitable→Unprofitable(6000)+zero-net-movement; profitable→exact predicted delta; min_profit boundary tight (succeed at predicted, revert at +1)
- [ ] 🟡 ★ `testing-5` **M1-GATE**: differential/property rounding-mirror test *(testing · 5d · testing-2,3, sizing-8, onchain-10)* — Raydium CPMM GREEN (LiteSVM): both-dirs + 256-case fuzz(reserves+fee+amount, 0 drift, minimal-counterexample shrink) + Token-2022 fee path (rounding_mirror_fuzz.rs + t22_fee.rs). Orca off-chain mirror DONE (sizing-5); residual: the Orca on-chain `swap_v2` differential + Surfpool real-venue
- [x] `testing-6` Trust-boundary + Token-2022 filter negative tests *(testing · 3d · testing-2,4, onchain-3,5)* — GREEN: non-allowlisted-program→6001 + foreign-owned-balance→6002 (trust_boundary.rs); all 7 BadExt mints rejected + plain/fee-only/display-only accepted (token2022_filter.rs)
- [ ] 🟡 `testing-7` CU / account-lock / tx-byte budget + ALT pre-warm asserts *(testing · 2d · testing-4, txbuilder-9)* — CU<1.4M measured GREEN (cu_budget.rs); locks<128/bytes≤1232/ALT-prewarm via txbuilder-6/9 (host-green); CU-baseline-regression fixture still TODO
- [ ] 🟡 `testing-8` Surfpool mainnet-fork integration vs real Raydium/Orca *(testing · 5d · testing-4,6, txbuilder-12)* — surfpool harness + dependency-free cheatcode RPC client + runtime-parity differential GREEN (surfpool_integration.rs). Real-VENUE differential harness BUILT (surfpool_real_venue.rs): lazy-forks the real Raydium CPMM pool/program/config/observation/vaults/mints over a Chainstack fork, decodes them (detection-3 offsets verified vs LIVE bytes), derives reserve=vault−protocol−fund, and runs the off-chain quote — all GREEN read-only. The live `swap_base_input` against the REAL program is gated (`ARBIT_REAL_VENUE_LIVE=1`) + BLOCKED: surfpool reverts `InvalidAccountData` @4391 CU inside the real Anchor program's account validation (raw zero-copy/spl deserialize on forked-account data; no Anchor log; amount/content-independent → surfpool substrate limit for forked Anchor programs). NEXT: load the real Raydium `.so` into LiteSVM (aligns accounts) for the bit-exact swap differential, or a surfpool fix. Orca = same shape.
...
- [x] `add-2` **HIGH** · Fase 1 · Inventory round-trip-closure invariant (leg-B out mint == leg-A in/base mint) *(onchain + signer + testing)* — GREEN: signer-side (`TxShapeValidator` rejects `RouteDoesNotCloseToBaseAta`) + on-chain assert (processor returns `RouteDoesNotClose`=6010 when the intermediate is not consumed back to baseline) + LiteSVM negative (closure.rs: a stranded-intermediate route reverts 6010 even when base grows)
...
- [ ] 🟡 `add-5` **MED** · Fase 1 · Runtime SIMD-0268/0339 feature-gate detection + measured CU-per-CPI budget *(onchain)* — `arb_config::features` (`FeatureGateState`/`CpiBudget::from_features`) owns the 5/9·1000/946·128/255 mapping; default = conservative pre-activation. Measured CU on LiteSVM GREEN (cu_budget.rs: real `compute_units_consumed`, <1.4M, not hardcoded). Residual: runtime activation READ (RPC getFeatureActivation / on-chain feature set) — Class C network
